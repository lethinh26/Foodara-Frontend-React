...
const OrderInbox: React.FC = () => {
  ...
  const handleReject = async () => {
    ...
    setOrders((prev) =>
      prev.map((o) =>
        o.id === rejectModal
          ? { ...o, status: "cancelled", cancelReason: rejectReason }
          : o,
      ),
    );
  ...
  const confirmed = orders.filter((o) => ["confirmed", "preparing", "picked_up", "picked_up"].includes(o.status));
  const completed = orders.filter((o) =>["delivered", "cancelled", "completed"].includes(o.status));
  ...
  const OrderCard: React.FC<{ order: Order }> = ({ order }) => (
    <Card
      style={{
        borderRadius: 12,
        marginBottom: 12,
        borderLeft: `4px solid ${ORDER_STATUS_COLORS[order.status]}`,
      }}
    >
      <div
        style={{
      ...
      >
        <div>
          ...
          <Text strong style={{ color: "var(--primary)" }}>
            {formatVND(order.pricing ? order.pricing.total : 0)}
          </Text>
          <Text type="secondary" style={{ marginLeft: 8, fontSize: 12 }}>
        ...
        {order.status === "confirmed" && (
          ...
          <Tag 
            color="processing" 
            icon={<Clock size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} />}
    ...
    <div className="animate-fade-in">
      ...
      <Tabs
        items={[
          ...
          {
            key: "confirmed",
            label: `Đang xử lý (${confirmed.length})`,
            children:
              confirmed.length === 0 ? (
          ...
          {
            key: "completed",
            label: `Hoàn tất (${completed.length})`,
            children:
              completed.length === 0 ? (
                <Empty description="Không có đơn" />
              ) : (
